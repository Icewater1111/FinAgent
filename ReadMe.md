...
pip install dashscope==1.23.8 pandas==2.3.1 numpy==2.3.1 requests==2.32.4 akshare==1.17.20 alpha_vantage==3.0.0 newsapi-python==0.2.7 pywebview==6.0
```
...
最后运行main_with_gradio.py文件

```python
python main_with_gradio.py
```

在弹出窗口中使用本工具，或在浏览器http://localhost:7860/中使用本工具。（默认端口为7860，可在main_with_gradio.py文件进行更改）
