...
# 输入和输出文件路径
rag_test_file_path = "rag_test_every_api.txt"
output_file_path = f"rag_test_every_api_results.json"
...
try:
    with (open(rag_test_file_path, 'r', encoding='utf-8') as file):
        ...
        while True:
            ...
            correct_api_line = file.readline()
            if not correct_api_line:
                print("警告: 文件意外结束，没有找到对应的正确API名称")
                break
                
            query = query_line.strip()
            correct_api_name = correct_api_line.strip()
            test_results["summary"]["total_tests"] += 1
            
            print(f"\n--- 处理第 {test_results['summary']['total_tests']} 个查询: '{query}' ---")
            
            # 准备记录当前测试结果
            current_test = {
                "query": query,
                "correct_api": correct_api_name,
                "processed_query": "",
                "top_k_results": [],
                "is_correct": False
            }
            ...
            current_test["processed_query"] = last_query
            
            query_index = extract_market_category(last_query)
            
            # 检索阶段
            results = faiss_databases[query_index].similarity_search_with_score(last_query, k=3)
            # results = faiss_key_databases[query_index].similarity_search_with_score(last_query, k=3)

            #不使用分类文档
            # results = all_database.similarity_search_with_score(last_query, k=3)
            ...
            for doc, score in results:
                ...
                if tool_meta_name:
                    rag_api_names.append(tool_meta_name)
                    current_test["top_k_results"].append({
                        "name": tool_meta_name,
                        "score": score,
                        # 只存储前100字符
            ...
            # 检查是否正确匹配
            if correct_api_name in rag_api_names:
                test_results["summary"]["correct_matches"] += 1
                current_test["is_correct"] = True
                print(f"√ 正确匹配: 检索结果中包含正确API '{correct_api_name}'")
            else:
                print(f"× 未匹配: 正确API '{correct_api_name}' 不在检索结果中")
                print(f"检索到的API: {rag_api_names}")
            
            # 将当前测试结果存入详细记录
            test_results["details"].append(current_test)
    
    # 计算召回率
    if test_results["summary"]["total_tests"] > 0:
        test_results["summary"]["recall_rate"] = (
            test_results["summary"]["correct_matches"] / test_results["summary"]["total_tests"]
        ) * 100
    
    # 将结果保存到JSON文件
    with open(output_file_path, 'w', encoding='utf-8') as out_file:
        json.dump(test_results, out_file, ensure_ascii=False, indent=4)
    
    print("\n--- 测试完成 ---")
    print(f"总测试数: {test_results['summary']['total_tests']}")
    print(f"正确匹配数: {test_results['summary']['correct_matches']}")
    print(f"召回率: {test_results['summary']['recall_rate']:.2f}%")
    ...
    # 如果发生错误，尝试保存已收集的结果
    if 'test_results' in locals() and test_results["summary"]["total_tests"] > 0:
        with open(output_file_path + ".partial", 'w', encoding='utf-8') as out_file:
            json.dump(test_results, out_file, ensure_ascii=False, indent=4)
        print(f"部分结果已保存到: {output_file_path}.partial")